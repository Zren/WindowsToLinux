...
function parse()
    if string.match( vlc.path, "list=" ) then
        ...
        local playlistIndex = tonumber(get_url_param( vlc.path, "index" ) or "1")
        ...
        while true do
            ...
            playlist_parsed = nil
            playlist_parsed = parse_playlist_xml(playlistData).root.video
...
function parse_playlist_xml(data)
    local tree = {}
    local stack = {}
    local tmp = {}
    local tmpTag = ""
